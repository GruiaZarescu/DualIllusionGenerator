...
namespace DualIllusionGenerator
{
    ...
    public static class VoxelMesher
    {
        ...
        private static readonly int[,] EdgeEndpoints = new int[12, 2]
        {
            {0,1},{1,2},{2,3},{3,0},
            {4,5},{5,6},{6,7},{7,4},
            {0,4},{1,5},{2,6},{3,7}
        ...
        public static MeshData Generate(VoxelGrid grid, float isoLevel = 0.5f)
        {
            ...
            for (int x = -1; x < grid.Width; x++)
                for (int y = -1; y < grid.Height; y++)
                    for (int z = -1; z < grid.Depth; z++)
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            int ix = x + CornerOffsetX[i];
                            int iy = y + CornerOffsetY[i];
                            int iz = z + CornerOffsetZ[i];
                            cube[i] = grid.GetVoxel(ix, iy, iz) ? 1f : 0f;
                        }
                        ...
                        for (int i = 0; i < 12; i++)
                        {
                            ...
                            {
                                int e1 = EdgeEndpoints[i, 0];
                                int e2 = EdgeEndpoints[i, 1];

                                float mu = (isoLevel - cube[e1]) / (cube[e2] - cube[e1]);

                                vertList[i] = new Vector3(
                                    (x + CornerOffsetX[e1] + mu * (CornerOffsetX[e2] - CornerOffsetX[e1])) * s,
                                    (y + CornerOffsetY[e1] + mu * (CornerOffsetY[e2] - CornerOffsetY[e1])) * s,
                                    (z + CornerOffsetZ[e1] + mu * (CornerOffsetZ[e2] - CornerOffsetZ[e1])) * s
                                );
                            }
                        }

                        var tri = TriangleTable[cubeIndex];
                        for (int i = 0; i < tri.Length && tri[i] != -1; i += 3)
                        {
                            int idx = mesh.Vertices.Count;
                            mesh.Vertices.Add(vertList[tri[i]]);
                            mesh.Vertices.Add(vertList[tri[i + 1]]);
                            mesh.Vertices.Add(vertList[tri[i + 2]]);
